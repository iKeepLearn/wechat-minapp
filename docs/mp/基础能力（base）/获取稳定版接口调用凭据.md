<!-- 来源: https://developers.weixin.qq.com/doc/service/api/base/api_getstableaccesstoken.html -->
...
本接口用于获取获取全局唯一后台接口调用凭据（Access Token），token 有效期为 7200 秒，但此接口和 [getAccessToken](api_getaccesstoken)  互相隔离，且比其更加稳定，推荐使用此接口替代。开发者需要进行妥善保存，使用注意事项请参考[此文档](https://developers.weixin.qq.com/doc/oplatform/developers/dev/AccessToken.html) 。

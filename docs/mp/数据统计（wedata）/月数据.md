<!-- 来源: https://developers.weixin.qq.com/doc/service/api/wedata/mess/api_getupstreammsgmonth.html -->
...
接口英文名：getupstreammsgmonth

本接口用于获取消息发送月数据

每个月的周期数据的数据标注日期在当月的第一天（当月1日或周一）。在某一月过后去调用接口，才能获取到该周期的数据。比如，在12月1日以（11月1日-11月5日）作为（begin_date和end_date）调用获取月数据接口，可以获取到11月1日的月数据（即11月的月数据）。
...
- 调用方法：officialAccount.datacube.getUpstreamMsgMonth

<!-- 来源: https://developers.weixin.qq.com/miniprogram/dev/server/API/VirtualPayment/api_query_adver_funds.html -->

# # 查询广告金发放记录

> 接口应在服务器端调用，不可在前端（小程序、网页、APP等）直接调用，具体可参考接口调用指南。

接口英文名：query_adver_funds
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| settle_begin | number | 否 | 查询结算周期开始时间，unix秒级时间戳 |
| settle_end | number | 否 | 查询结算周期结束时间，unix秒级时间戳 |
| fund_type | number | 否 | (可选)广告金发放原因， 0:通用赠送，1:广告激励，2:定向激励 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| adver_funds_list | objarray | 广告金发放记录列表 |

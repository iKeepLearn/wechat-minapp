<!-- 来源: https://developers.weixin.qq.com/miniprogram/dev/server/API/VirtualPayment/api_create_withdraw_order.html -->

# # 创建提现单

> 接口应在服务器端调用，不可在前端（小程序、网页、APP等）直接调用，具体可参考接口调用指南。

接口英文名：create_withdraw_order
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| withdraw_no | string | 是 | 提现单单号，长度为[8,32]，字符只允许使用字母、数字、'\_'、'-' |
| withdraw_amount | string | 是 | 提现的金额，单位元，例如提现1分钱请使用0.01，允许不传，不传的情况下表示全额提现 |

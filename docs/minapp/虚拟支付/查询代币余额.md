<!-- 来源: https://developers.weixin.qq.com/miniprogram/dev/server/API/VirtualPayment/api_query_user_balance.html -->

# # 查询代币余额

> 接口应在服务器端调用，不可在前端（小程序、网页、APP等）直接调用，具体可参考接口调用指南。

接口英文名：query_user_balance
...
| user_ip | string | 是 | 用户ip，例如:1.1.1.1 |
...
| present_balance | number | 赠送账户的代币余额 |
| sum_save | number | 累计有价货币充值数量 |
| sum_present | number | 累计赠送无价货币数量 |
| sum_balance | number | 历史总增加的代币金额 |
| sum_cost | number | 历史总消耗代币金额 |
| first_save_flag | boolean | 是否满足首充活动标记。0:不满足。1:满足 |

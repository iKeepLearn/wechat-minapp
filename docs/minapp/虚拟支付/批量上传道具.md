<!-- 来源: https://developers.weixin.qq.com/miniprogram/dev/server/API/VirtualPayment/api_start_upload_goods.html -->

# # 批量上传道具

> 接口应在服务器端调用，不可在前端（小程序、网页、APP等）直接调用，具体可参考接口调用指南。

接口英文名：start_upload_goods
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| upload_item | objarray | 是 | 上传的商品列表 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| id | string | 是 | 道具id，长度(0,20]，字符只允许使用字母、数字、'\_'、'-'，中文算一个字符 |
| name | string | 是 | 道具名称，长度(0，20] |
| price | number | 是 | 道具单价，单位分，需要大于0 |
| remark | string | 是 | 道具备注，长度(0,1024] |
| item_url | string | 是 | 道具图片的url地址，当前仅支持jpg,png等格式 |

<!-- 来源: https://developers.weixin.qq.com/miniprogram/dev/server/API/VirtualPayment/api_currency_pay.html -->
...
| user_ip | string | 否 | 用户ip，例如:1.1.1.1 |
| amount | number | 否 | 支付的代币数量 |
| order_id | string | 否 | 订单号 |
| payitem | string | 否 | 物品信息。记录到账户流水中。如:[{"productid":"物品id", "unit_price": 单价, "quantity": 数量}] |
| remark | string | 否 | 备注 |
...
| used_present_amount | number | 使用赠送部分的代币数量 |

<!-- 来源: https://developers.weixin.qq.com/miniprogram/dev/server/API/VirtualPayment/api_cancel_currency_pay.html -->

# # 代币支付退款

> 接口应在服务器端调用，不可在前端（小程序、网页、APP等）直接调用，具体可参考接口调用指南。

接口英文名：cancel_currency_pay
...
| user_ip | string | 是 | 用户ip，例如1.1.1.1 |
| pay_order_id | string | 是 | 代币支付(调用currency_pay接口时)时传的order_id |
| order_id | string | 是 | 本次退款单的单号 |

<!-- 来源: https://developers.weixin.qq.com/miniprogram/dev/server/API/weixin-express/same_city_distribution/api_intracity_storecharge.html -->

# # 门店运费充值

> 接口应在服务器端调用，不可在前端（小程序、网页、APP等）直接调用，具体可参考接口调用指南。

接口英文名：intracity_storecharge
...
| service_trans_id | string | 是 | 运力ID | 枚举值 |
| amount | number | 是 | 充值金额 单位：分, 50元起充 | - |
| pay_mode | string | 否 | 充值主体	门店：PAY_MODE_STORE；小程序:PAY_MODE_APP；服务商：PAY_MODE_COMPONENT，不传pay_mode默认pay_mode=PAY_MODE_STORE | - |
...
| wx_store_id | string | 微信门店编号，请求成功时返回 |

<!-- 来源: https://developers.weixin.qq.com/miniprogram/dev/server/API/cloudbase/database/api_exportdatabaseitem.html -->
...
接口英文名：exportDatabaseItem

通过本接口可以进行数据库导出.使用过程中如遇到问题，可在[开放平台服务商专区](https://developers.weixin.qq.com/community/minihome/mixflow/1355698687267438595) 发帖交流。
...
| --- | --- | --- | --- | --- |
| env | string | 是 | 云环境ID | - |
| file_path | string | 是 | 导出文件路径（文件会导出到公共的云存储中，可使用getDownloadTcbFileLink接口获取下载链接） | - |
| file_type | number | 是 | 导出文件类型，文件格式参考importDatabaseItem接口的文件格式部分 | 枚举值 |
| query | string | 是 | 导出条件 | - |
...
| errcode | number | 错误码 |
| errmsg | string | 错误信息 |
| job_id | number | 导出任务ID，使用getDatabaseMigrateStatus接口查询导出结果，获取文件下载链接。 |

## # 4. 枚举信息

### # Body.file_type Enum

导出文件类型，文件格式参考[importDatabaseItem接口](api_importdatabaseitem) 的文件格式部分

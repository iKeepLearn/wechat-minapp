...
use std::env;
use std::sync::Arc;
use wechatmp::mp_message::{SendMessageArgs, TemplateMessage};
use wechatmp::{MemoryTokenStorage, StableToken};
use wechatmp::{ReqwestHttpClient, WechatMp};

/// 初始化测试客户端
fn setup_client() -> WechatMp {
    dotenv().ok();
    ...
    WechatMp::custom(http_client, token_storage)
}
